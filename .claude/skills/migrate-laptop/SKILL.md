...
| `export`           | Phase 2 — run `zipstuff` + `laptop-migrate.sh export-apps`, copy to USB |
| `verify`           | Phase 3 — decrypt-test USB archives |
| `bootstrap`        | Phase 4 — Xcode CLT → Homebrew → clone → `laptop-migrate.sh new-laptop` |
| `import`           | Phase 5 — unpack USB archives into `~` |
| `reconnect`        | Phase 6 — login flows + verifications |
| `refresh-manifest` | Regenerate `laptop-migration/Brewfile` + `laptop-migration/*.txt` (shortcut for Phase 1 step 3) |
...
4. **Dry-run by default** when `laptop-migrate.sh` supports it. Only drop `--dry-run` after the user confirms.
...
- Never `git add` anything matching the `SENSITIVE — NEVER COMMIT` section of `laptop-migration/LAPTOP_MIGRATION_MANIFEST.md`. If a `git add` call matches, **refuse and flag**.
